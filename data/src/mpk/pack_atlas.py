...
def export_image(
    ...
    for i, image in enumerate(images):
        ...
        new_image.paste(image["cropped_data"], real_position)
...
def main(size_factor, atlas_unpack_path, target_path):
    ...
    log_info("Read prefix.txt...", is_verbose=True)
    ...
    log_info("Load images...", is_verbose=True)
    for image_path in image_paths:
        log_debug(f"Load {image_path}...")
        ...
        if offset_sprite_match:
            x_offset = int(offset_sprite_match.group(2))
            y_offset = int(offset_sprite_match.group(3))
            w_offset = int(offset_sprite_match.group(4))
            h_offset = int(offset_sprite_match.group(5))
            image_path = image_path.parent / image_path.name.replace(
                offset_sprite_match.group(1), ""
            )
        ...
        images.append(
            {
                ...
                "sourceSize": {"h": height + h_offset, "w": width + w_offset},
                "spriteSourceSize": {"x": left + x_offset, "y": top + y_offset},
            }
        )
    ...
    log_debug("Target size = {max_size}...")

    image_sizes = [
        (image["frame"]["w"] + (padding * 2), image["frame"]["h"] + (padding * 2), i)
        for i, image in enumerate(images)
    ]
    new_image_names = []

    log_info("Calculate packer...", is_verbose=True)
    packer = newPacker(mode=PackingMode.Offline, rotation=False)
    for image_size in image_sizes:
        packer.add_rect(*image_size)
    packer.add_bin(
        target_size - (padding * 2), target_size - (padding * 2), float("inf")
    )
    packer.pack()

    log_info("Generate packed images...", is_verbose=True)
    group_id = 0
    for abin in packer:
        image_group = []
        positions = []
    ...
    log_info("Write atlas...", is_verbose=True)
    ...
    log_info("Pack complete", is_verbose=True)


if __name__ == "__main__":
    if len(sys.argv) < 2:
        log_error("You must pass a single atlas_unpack path")
        sys.exit(-1)
    if len(sys.argv) > 4:
        log_error("Too many args")
        sys.exit(-1)
    if len(sys.argv) > 3:
        size_factor_param = int(sys.argv[3])
    else:
        size_factor_param = 8
    if len(sys.argv) > 2:
        target_path_param = sys.argv[2]
    else:
        target_path_param = ""

    atlas_unpack_path_param = sys.argv[1]

    if not target_path_param:
        target_path_param = str(Path(atlas_unpack_path_param).parent)
    main(size_factor_param, atlas_unpack_path_param, target_path_param)
